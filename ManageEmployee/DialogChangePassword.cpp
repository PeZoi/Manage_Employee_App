...
void DialogChangePassword::handleChangePassword() {
	...
	if (oldPassword == newPassword) {
		ErrorLabel* error = new ErrorLabel(tr("  Old password and new password are the same"));
		error->showTemporary(ui->verticalLayout, 3000);
		return;
	}

	if (newPassword != retypeNewPassword) {
		ErrorLabel* error = new ErrorLabel(tr("  Retyped new password does not match the new password"));
		error->showTemporary(ui->verticalLayout, 3000);
		return;
	}
	...
	if (!DatabaseSingleton::getInstance()->getDB()->getEmployeeRepository()->changePasswordAdmin(newPassword)) {
		ErrorLabel* error = new ErrorLabel(tr("  Change password failed!"));
		error->showTemporary(ui->verticalLayout, 3000);
		return;
	}
